...
#define TAG "ti.keyframes"

using namespace v8;

static Persistent<Object> bindingCache;

static void TiKeyframes_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "TiKeyframes.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::TiKeyframesBindings::lookupGeneratedInit(
...
static void TiKeyframes_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, TiKeyframes_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void TiKeyframes_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TiKeyframesBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TiKeyframesBinding = {
	"ti.keyframes",
	TiKeyframes_init,
	TiKeyframes_dispose
};
...
Java_ti_keyframes_TiKeyframesBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.keyframes", &TiKeyframesBinding);
	titanium::KrollBindings::addExternalLookup(&(::TiKeyframesBindings::lookupGeneratedInit));
}
