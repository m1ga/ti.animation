...
Persistent<FunctionTemplate> KeyframesProxy::proxyTemplate;
jclass KeyframesProxy::javaClass = NULL;

KeyframesProxy::KeyframesProxy(jobject javaObject) : titanium::Proxy(javaObject)
{
}

void KeyframesProxy::bindProxy(Local<Object> exports, Local<Context> context)
{
	Isolate* isolate = context->GetIsolate();

	Local<FunctionTemplate> pt = getProxyTemplate(isolate);
	Local<Function> proxyConstructor = pt->GetFunction(context).ToLocalChecked();
	Local<String> nameSymbol = NEW_SYMBOL(isolate, "Keyframes"); // use symbol over string for efficiency
	exports->Set(nameSymbol, proxyConstructor);
}

void KeyframesProxy::dispose(Isolate* isolate)
{
	LOGD(TAG, "dispose()");
...
Local<FunctionTemplate> KeyframesProxy::getProxyTemplate(Isolate* isolate)
{
	if (!proxyTemplate.IsEmpty()) {
	...
	javaClass = titanium::JNIUtil::findClass("ti/keyframes/KeyframesProxy");
	EscapableHandleScope scope(isolate);

	// use symbol over string for efficiency
	Local<String> nameSymbol = NEW_SYMBOL(isolate, "Keyframes");
...
, javaClass, nameSymbol);
	...
	t->Set(titanium::Proxy::inheritSymbol.Get(isolate),
		FunctionTemplate::New(isolate, titanium::Proxy::inherit<KeyframesProxy>)->GetFunction());
	...
	titanium::SetProtoMethod(isolate, t, "getFrameCount", KeyframesProxy::getFrameCount);
	...
	titanium::SetProtoMethod(isolate, t, "getFrameRate", KeyframesProxy::getFrameRate);
	titanium::SetProtoMethod(isolate, t, "pauseAnimation", KeyframesProxy::pauseAnimation);
	...
	instanceTemplate->SetAccessor(NEW_SYMBOL(isolate, "message"),
			KeyframesProxy::getter_message,
			KeyframesProxy::setter_message,
			Local<Value>(), DEFAULT,
...
// Methods --------------------------------------------------------------------
void KeyframesProxy::stopAnimation(const FunctionCallbackInfo<Value>& args)
{
	LOGD(TAG, "stopAnimation()");
	...
	if (!methodID) {
		methodID = env->GetMethodID(KeyframesProxy::javaClass, "stopAnimation", "()V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'stopAnimation' with signature '()V'";
			LOGE(TAG, error);
...
void KeyframesProxy::getMessage(const FunctionCallbackInfo<Value>& args)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(KeyframesProxy::javaClass, "getMessage", "()Ljava/lang/String;");
		if (!methodID) {
...
void KeyframesProxy::getFrameCount(const FunctionCallbackInfo<Value>& args)
{
	LOGD(TAG, "getFrameCount()");
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(KeyframesProxy::javaClass, "getFrameCount", "()I");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'getFrameCount' with signature '()I'";
			LOGE(TAG, error);
				titanium::JSException::Error(isolate, error);
				return;
		}
...
void KeyframesProxy::setMessage(const FunctionCallbackInfo<Value>& args)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(KeyframesProxy::javaClass, "setMessage", "(Ljava/lang/String;)V");
		if (!methodID) {
...
void KeyframesProxy::startAnimation(const FunctionCallbackInfo<Value>& args)
{
	LOGD(TAG, "startAnimation()");
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(KeyframesProxy::javaClass, "startAnimation", "()V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'startAnimation' with signature '()V'";
			LOGE(TAG, error);
				titanium::JSException::Error(isolate, error);
				return;
		}
...
void KeyframesProxy::resumeAnimation(const FunctionCallbackInfo<Value>& args)
{
	LOGD(TAG, "resumeAnimation()");
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(KeyframesProxy::javaClass, "resumeAnimation", "()V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'resumeAnimation' with signature '()V'";
			LOGE(TAG, error);
				titanium::JSException::Error(isolate, error);
				return;
		}
...
void KeyframesProxy::printMessage(const FunctionCallbackInfo<Value>& args)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(KeyframesProxy::javaClass, "printMessage", "(Ljava/lang/String;)V");
		if (!methodID) {
...
void KeyframesProxy::getFrameRate(const FunctionCallbackInfo<Value>& args)
{
	LOGD(TAG, "getFrameRate()");
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(KeyframesProxy::javaClass, "getFrameRate", "()I");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'getFrameRate' with signature '()I'";
			LOGE(TAG, error);
				titanium::JSException::Error(isolate, error);
				return;
		}
...
void KeyframesProxy::pauseAnimation(const FunctionCallbackInfo<Value>& args)
{
	LOGD(TAG, "pauseAnimation()");
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(KeyframesProxy::javaClass, "pauseAnimation", "()V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'pauseAnimation' with signature '()V'";
			LOGE(TAG, error);
				titanium::JSException::Error(isolate, error);
				return;
		}
...
void KeyframesProxy::getter_message(Local<Name> property, const PropertyCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
...
void KeyframesProxy::setter_message(Local<Name> property, Local<Value> value, const PropertyCallbackInfo<void>& args)
{
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
